  ...
  function ModalHeader() {
    return (
      <div className="bg-gray-100 px-4 pt-5 pb-4 sm:p-6 sm:pb-4 relative flex items-center">
        ...
        <h2 className={`${isMobile ? "text-1xl" : "text-2xl"}`}>Solicitar orçamento para {selectedProduct?.supplier.companyName}</h2>{" "}
              ...
              <div className="flex flex-wrap gap-2 mt-1">
                {productVariation.productVariationOptions.map((option: ProductVariationOption) => {
                  ...
                  selectedVariations.forEach((item) => {
                    if (item.variationId === productVariation.variation.id && item.variationOptionId === option.variationOption.id) {
                      exists = true;
                    }
                  });
                  return (
                  ...
                  {isMobile ? (
                    <div className="flex flex-col mr-4 ">
                      ...
                      <div className="flex items-center">
                        <span className="label-text">Quantidade: {count} unidades </span>
                      </div>
                      {selectedProduct?.productsVariations?.length! > 0 && <VariationOptions />}
                    </div>
                  ) : (
                    <div className="flex flex-col w-64 h-64 mr-4 ">
                      <img src={selectedProduct?.image} alt="product image" className="w-64 h-64 rounded-lg" /> <ProductCouter />
                      <div className="flex items-center">
